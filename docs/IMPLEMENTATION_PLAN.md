...
| 21  | `nampinonychus/veto.py`           | 買いを止める権利だけを LLM に渡す（段階2。既定は無効）       |
...
| 判断の主体     | 決定的コード。LLM は**買いを止める方向にだけ**関与できる（`veto`。既定は無効） |
...
- LLM を呼ぶのは**記録の言語化**（`narrate.py`）と**拒否権**（`veto.py`）の2か所。
  言語化は所感と学びを書くだけで判断に関与せず、書けなくても運用は続く。
  拒否権は買いを出す回にだけ通し、**止める方向にしか関与しない**（段階2）。
- モデルは `agent.yaml` の `narrate.model` / `narrate.effort` で指定する。
  Fable・Mythos 系は料金が上位帯のため `config.py` が受け付けない。

#### 判断を LLM へ移すこと（方針。未着手）

判断を決定的コードに置いたのは、**最初の段階としてそうした**のであって、
最終形ではない。目指しているのは「**破れない制約の中で、LLM が臨機応変に振る舞う**」形である。

そのための下ごしらえは済んでいる。

- `decide()` は I/O を持たない純関数で、入力（観測・状態）と出力（取消・発注・理由）が
  はっきりしている。**同じ契約のまま中身を差し替えられる**
- リスク制約（取得原価 60%、現金 20%、-15% 冬眠、-25% 撤退）は `decide()` の外側にも
  効いており、判断の中身が変わっても最悪ケースは動かない
- 評価軸（Buy&Hold 比較・最大ドローダウン）が揃っているため、
  **LLM 版が決定的版より良くなったかを同じ物差しで測れる**
...
| --- | --- | --- |
| 1（実装済み） | 所感・学びの言語化 | なし |
| 2（実装済み。既定は無効） | **HOLD する権利だけ**を与える | 発注を止める方向のみ |
| 3 | パラメータ改善の提案（人間が `agent.yaml` へ反映） | 間接的 |
| 4 | 制約の内側で、段の置きかたや利確の判断を任せる | 直接 |

2 は「決定的コードが買うと決めたあとに通し、止める権利だけ渡す」形。
買わせる方向には関与させないため、最悪ケースが変わらない。`veto.py` に実装済みで、
`agent.yaml` の `veto.enabled` で切り替える。**既定は無効**とし、
有効にするかどうかは人間が決める。

- 諮るのは `action` が BUY の回だけ。利確の売り・時間切れ・強制手仕舞い・
  冬眠の取消は諮らない。それらを止めるのは建玉を抱え続ける方向であり、安全側ではない
- 答えは `PROCEED` か `STOP: 理由` の二値。数量・価格・段数を動かす余地を渡さない
- 止めた回は**取消も発注もしない**。板の状態を変えないまま次の回へ持ち越す。
  1段目の置き直しを止めた場合、いまの注文はそのまま板に残る
  （止められるのは新しく出す注文であって、すでに板にある注文ではない）
- 呼べなかった・答えを読めなかったときは `veto.on_failure` に従う。
  `hold` なら見送り、`proceed` なら決定的コードの判断をそのまま通す。既定は `hold`
- 諮った結果は判断ログの `veto` に残す。諮らなかった回は `null`

いずれの段階でも、**`CLAUDE.md` の不変のルールは変わらない。**
実取引を行わない、判断できないときは HOLD、戦略・リスクの値をエージェント自身が
書き換えない。3 で提案させるのは人間が承認するための材料であって、
自動反映ではない。
...
- **アンカーには進行中の未確定足を含める。** 含めることで、直近高値の更新中は
...
- Phase 6 の売買判断は決定的コードで行う。LLM に渡すのは買いを止める権利だけで
  （段階2、既定は無効）、買わせる方向には関与させない。
...
- **運用の産物は `var/` 配下にまとめ、Git では追跡しない。**
  ...
  実行のたびに作業ツリーが汚れると、15分ごとの運用で git の操作が止まるため
  （旧「未確定事項2」の結論）
