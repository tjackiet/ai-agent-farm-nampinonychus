...
| 21  | `nampinonychus/veto.py`           | 買いを止める権利だけを LLM に渡す（段階2）                   |
...
| 判断の主体     | 決定的コード。LLM は**買いを止める方向にだけ**関与する（`veto`） |
...
- リスク制約（取得原価 90%、現金 10%、-15% 冬眠、-25% 撤退）は `decide()` の外側にも
...
| --- | --- | --- |
| 1（実装済み） | 所感・学びの言語化 | なし |
| 2（稼働中。2026-08-23〜） | **HOLD する権利だけ**を与える | 発注を止める方向のみ |
...
`agent.yaml` の `veto.enabled` で切り替える。有効にするかどうかは人間が決める。
**2026-08-23 に利用者の判断で有効にした。** 扱うのはペーパー口座だけなので、
最悪でもペーパーの損益が動くにとどまる。
