...
7段の階段を用意します。1段目だけアンカー基準、2段目以降は**直前の約定価格**基準です。
...
- **段の間隔を下ほど広げる**（0.7→0.8→1.0→1.3→1.7→2.1→2.6%）。下落が続くほど慎重になります。
- **予算は緩やかにしか増やさない**（105k→145k、1.4倍未満）。倍々に張るマーチンゲールは採りません。
- **合計は初期資金の 90%**。残り 10% は使いません（`risk-policy.md`）。

15分足の値幅は中央値 0.090%、1日の値幅は中央値 1.77% でした（同じ実測）。
1段目の -0.7% は日常的に届き、7段すべてを使うにはアンカーから -9.78% の下落が要ります。
...
なお、2段目以降の指値価格は**直前の約定価格**から決まります。したがって、
**直前の段が約定するまで、次の段の価格は決まりません。** 板に未約定の買い指値が
あるうちは、次の段を出しません。

そのため実際に板へ置ける買い指値は**常に1本**です。`max_pending_buy_orders` の 2 は
上限として残しますが、通常は届きません。

ここを「最後に約定した価格」で代用してはいけません。2段目が板にあるまま3段目を
その価格で組み立てると、段の間隔が設計より詰まり（例：-0.5% と -0.7% で 0.2% しか
離れない）、資金を早く・高い位置で使ってしまいます。買い指値が2本以上ある状態を
見つけたら、市場に近い1本だけ残して取り消します。

### 1段目はアンカーに追従させる

1段目はアンカー基準なので、アンカーが動けば正しい指値価格も動きます。置いたまま
放置すると、板にある注文が戦略を表さなくなります。

- アンカーが**切り上がる**と、指値は下に取り残されて約定しなくなる
- アンカーが**切り下がる**と、指値は上に残り、ルールより高い位置で買ってしまう

そこで毎回、いまのアンカーから正しい価格を計算し直し、板の価格とのずれが
`reprice_threshold_pct`（0.1%）を超えていれば、取り消して置き直します。

- **2段目以降は動かしません。** 直前の約定価格が基準であり、アンカーとは無関係です。
- 予算や刻みが足りずに置き直せないときは、**いまの注文をそのまま残します。**
  取り消しただけで終わると、ただの機会損失になります。

### 想定例（アンカー = 10,000,000 JPY）

| 段  | 指値価格  | 予算    | 数量   |
| --- | --------- | ------- | ------ |
| 1   | 9,930,000 | 105,000 | 0.0105 |
| 2   | 9,850,560 | 115,000 | 0.0116 |
| 3   | 9,752,054 | 125,000 | 0.0128 |
| 4   | 9,625,277 | 130,000 | 0.0135 |
| 5   | 9,461,647 | 140,000 | 0.0147 |
| 6   | 9,262,952 | 140,000 | 0.0151 |
| 7   | 9,022,115 | 145,000 | 0.0160 |

7段すべて約定した場合：取得原価 約 896,610 JPY / 0.0942 BTC、
**平均取得単価 約 9,518,149 JPY**（アンカーから -4.82%）。
階段を使い切るのはアンカーから -9.78% まで落ちたときです。
...
上の例なら +0.5% は 9,817,556 JPY（アンカーから -1.82%）、
+2.0% は 9,964,086 JPY（同 -0.36%）です。
ナンピンで下がった平均取得単価のぶん、アンカーまで戻らなくても利確できます。

**1段目で 30% だけ降ろし、残りは伸ばします。** 早く確定させるより、届いたときの
1回を大きく取る形です。届かなかった回は保有上限（3日）の成行になり、そこだけ
taker 0.1% と含み損の確定がかかります。**この負けを含めても勝つかどうか**が、
この幅を選ぶ根拠です。

実データ（2026-08-19〜09-14、閉じた 13 ラウンド）での到達率は次のとおりでした。

| 平均取得単価からの上昇 | 24時間以内の到達 |
| ---------------------- | ---------------- |
| +0.5%                  | 13 / 13          |
| +1.0%                  | 11 / 13          |
| +2.0%                  | 11 / 13          |

+1.0% に届く回は +2.0% にも届いています。**途中で降りる意味が薄い**ということです。
届かない 2 回が保有上限の成行になる損を含めた概算でも、+0.3/+0.6 の 7,593 JPY に
対し 12,027 JPY でした（`scripts/measure_take_profit.py`）。

**入りも出も指値なら往復コストは 0 です**（maker 0%）。成行を使うのは時間切れと
強制手仕舞いだけで、そのときだけ taker 0.1% がかかります。

なお、この根拠は 13 ラウンド・約1か月の一相場ぶんです。上げ相場に寄っている
可能性があり、下げ相場では届かない回が増えます。ペーパートレードでの試行として
選んだ値であり、確定した最適値ではありません。

利確注文も**指値**で常時板に置きます。買いが約定して平均取得単価が動いたら、
既存の売り指値をキャンセルして置き直します。

**時間切れ**：建玉の保有が 3 日を超えたら、損益に関わらず全量手仕舞いします。
運用前の実測（2026-07-19〜08-18 の15分足）では +0.3% への到達の中央値が 2.88 時間、
24時間以内の到達が 72.9% でした。運用後の実データでも、24時間以内に届かなかった
回は72時間以内にも届いていません。3日戻らない建玉は、この戦略が想定する値動きから
外れています。
...
stateDiagram-v2
    ...
    LADDERING --> LADDERING: 2〜7段目が約定
    LADDERING --> HOLDING: 7段すべて使用
    LADDERING --> IDLE: 全量利確
...
`HIBERNATING`（冬眠）では新規買いを止め、板に残っている買い指値も取り消します。
利確の売り指値は残します。
